...
model.load_weights("models/best_model_baseline.weights.h5")
model.compile(optimizer=keras.optimizers.Adam(learning_rate=config.LEARNING_RATE))

callbacks = [
    keras.callbacks.ModelCheckpoint("best_model_weighted.weights.h5", save_best_only=True, save_weights_only=True, monitor='val_loss'),
    keras.callbacks.EarlyStopping(patience=5, restore_best_weights=True, monitor='val_loss'),
    keras.callbacks.CSVLogger("training_log_weighted.csv"),
]
